...
{
    public sealed class GameStatsListPanel
    {
        private const float SearchBarWidth = 260f;
        private const float SectionSpacing = 12f;
        private const float InfoLabelHeight = 30f;
        ...
        {
            var infoRect = new Rect(rect.x, rect.y, rect.width, InfoLabelHeight);
            DrawInfoLabel(infoRect);

            var searchRect = new Rect(rect.x, infoRect.yMax + 4f, SearchBarWidth, 24f);
            var buttonsWidth = 220f;
            if (rect.width > SearchBarWidth + buttonsWidth + 12f)
            {
                var buttonsRect = new Rect(rect.xMax - buttonsWidth, searchRect.y, buttonsWidth, 24f);
                DrawExpandCollapseButtons(buttonsRect);
            }
            DrawSearchBar(searchRect);

            var listRect = new Rect(rect.x, searchRect.yMax + SectionSpacing, rect.width, rect.height - InfoLabelHeight - 4f - searchRect.height - SectionSpacing);
            var groupedStats = GetGroupedStats();
        ...
        private void DrawExpandCollapseButtons(Rect rect)
        {
        ...
        private void DrawInfoLabel(Rect rect)
        {
            var previousAnchor = Text.Anchor;
            ...
            Widgets.Label(rect, "RimMetrics.UI.GameStatsSourceHint".Translate());
            Text.Anchor = previousAnchor;
        ...
        private List<StatsGroupingService.CategoryGroup> GetGroupedStats()
        {
            ...
            return gameStats.TryGetGameGroupedStats(out var groupedStats)
                ? groupedStats
        ...
        {
            var gameStats = Current.Game?.GetComponent<GameComponent_GameStats>();
            return gameStats != null && gameStats.IsWaitingForGameGroupedStats();
        }
